...
// Create the Palette type
export const createPalette = (jsonString: string, onError?: OnError): Palette => {
    const raw = validatePaletteJSON(jsonString, onError);
    ...
    for (let i = 0; i < size; i++) {
        ...
        names[i] = entry.name;
        if (!isRGBA(entry.rgba)) onError?.("@createPalette : entry is not a RGBA value");
        rgba[i] = entry.rgba;
...
// Validate the JSON palette
const validatePaletteJSON = (jsonString: string, onError?: OnError): any => {
    let raw: any;
    try {
        raw = JSON.parse(jsonString);
    } catch (e: any) {
        onError?.("@createPalette : JSON is invalid");
    }
    // Okay validate 
    if (typeof raw !== "object" || raw === null) onError?.("@validatePalette : JSON must be an object with paletteName and entries");
    if (!raw.paletteName) onError?.("@validatePalette : Palette must have paletteName");
    if (typeof raw.paletteName !== "string") onError?.("@validatePalette : paletteName must be a string");
    if (!raw.entries) onError?.("@validatePalette : Palette must have entries");
    if (!Array.isArray(raw.entries)) onError?.("@validatePalette : entries must be an array");
    if (raw.entries.length === 0) onError?.("@validatePalette : entries array is empty");
    // Raw is okay 
    return raw;
}
